...
`.github/workflows/deploy.yml` construit `index.html` puis **déploie sur GitHub Pages via
GitHub Actions**, sérialisé par `concurrency` (→ fini les conflits « Deployment failed,
try again later »). Il ne se déclenche que sur un changement de **code** (`src/**`,
`CNAME`) — plus sur les commits `Update data.json`.
...
`data.json` est commité toutes les ~30 min par *Fetch Temperature Data*. L'app le lit
**directement depuis raw GitHub** (`raw.githubusercontent.com/.../main/data.json`), qui
reflète chaque commit sans redéploiement. Les mises à jour de données ne déclenchent donc
plus aucun build/déploiement ; `./data.json` (copie du build) ne sert que de repli.
